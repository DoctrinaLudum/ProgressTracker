...
# --- NOVA FUNÇÃO PARA CALENDÁRIO CUSTOMIZADO COM BUFFS SELECIONADOS ---
def generate_custom_season_calendar(user_selected_buff_names=None):
    calendar_data_list = []
    log.info(f"Iniciando geração do calendário customizado com buffs: {user_selected_buff_names}")
    if user_selected_buff_names is None:
        user_selected_buff_names = []
    ...
    try:
        ...
        if not all([shop_items_global, buff_definitions_global, activity_rules_global]):
            log.error("Erro crítico: Configs essenciais não encontradas ou vazias para calendário customizado.")
            return []
    except Exception as e: 
        log.error(f"Erro ao carregar configs ou datas para calendário customizado: {e}")
        return []
    ...
    buffs_for_current_bounties_week = set()
    vip_bonus_val = 0
    if SIM_IDEAL_PLAYER_HAS_VIP: # Esta simulação ainda respeita o VIP global
        vip_def = buff_definitions_global.get("vip", {})
        vip_bonus_val = vip_def.get("bonus_value", 0)

    # Rastreia bônus de todos os buffs definidos, não apenas os da prioridade
    all_buffs_to_track_totals = set(buff_definitions_global.keys()) - {"vip"}
    ...
    while current_date_iterator <= end_date_obj:
        ...
        # --- Lógica de Ganhos Diários (similar à original, mas adaptada) ---
        # (Esta parte é extensa e muito parecida com a original. Para economizar espaço no diff,
        # imagine que a lógica de cálculo de ganhos base, VIP, e bônus de itens
        # é copiada aqui, usando `active_buffs_at_day_start` que será influenciado
        # pelas compras de buffs selecionados pelo usuário.)
        # A principal diferença estará no loop de compra abaixo.

        # --- Loop de Compra e Ativação (MODIFICADO PARA BUFFS SELECIONADOS) ---
        action_taken_this_day_in_purchase_loop = True 
        while action_taken_this_day_in_purchase_loop:
            action_taken_this_day_in_purchase_loop = False 
            current_owned_for_unlock_check = set(purchased_buff_items_globally.keys()) | set(purchased_unlock_items_globally.keys())
            
            # Itera sobre os buffs que o USUÁRIO selecionou e que ainda não foram comprados
            for target_buff_item_name in user_selected_buff_names:
                if purchased_buff_items_globally.get(target_buff_item_name): continue 

                target_buff_shop_info = shop_items_global.get(target_buff_item_name)
                if not target_buff_shop_info:
                    log.warning(f"[CustomCal] Item de buff selecionado '{target_buff_item_name}' não encontrado na loja.")
                    continue 
                ...
                # 1. Tenta desbloquear o Tier do buff selecionado, se necessário
                if not unlocked_tiers_state.get(required_tier):
                    needs_unlock, items_to_buy_list, cost_to_unlock, _ = get_tier_unlock_status_and_cost(
                        required_tier, shop_items_global, seasonal_token_name_global, current_owned_for_unlock_check
                    )
                    if needs_unlock and cost_to_unlock != float('inf') and current_token_balance >= cost_to_unlock:
                        ...
                        for item_dict_to_buy in items_to_buy_list:
                            ...
                            current_owned_for_unlock_check.add(item_dict_to_buy['name']) 
                            daily_log_entry["purchases_today"].append({'name': item_dict_to_buy['name'], 'cost': item_dict_to_buy['cost'], 'type': f'unlock_T{required_tier}'})
                            # Se um item de desbloqueio também for um buff selecionado, marca como comprado
                            if item_dict_to_buy['name'] in user_selected_buff_names and not purchased_buff_items_globally.get(item_dict_to_buy['name']):
                                purchased_buff_items_globally[item_dict_to_buy['name']] = True 
                        unlocked_tiers_state[required_tier] = True
                        action_taken_this_day_in_purchase_loop = True
                        log.debug(f"[CustomCal] Dia {day_in_season_counter}: Desbloqueado Tier {required_tier} para {target_buff_item_name}. Custo: {cost_to_unlock}")
                        break # Reinicia o loop de compra para reavaliar prioridades
                
                if action_taken_this_day_in_purchase_loop: break # Se uma ação de desbloqueio foi tomada, reavalia

                # 2. Tenta "comprar" o buff selecionado se o tier estiver desbloqueado
                if unlocked_tiers_state.get(required_tier) and not purchased_buff_items_globally.get(target_buff_item_name):
                    if is_sfl_item: # Itens SFL são "comprados" sem custo de token sazonal
                        log.info(f"[CustomCal] Dia {day_in_season_counter}: Ativando buff SFL selecionado '{target_buff_item_name}' (Tier {required_tier}).")
                        purchased_buff_items_globally[target_buff_item_name] = True
                        daily_log_entry["purchases_today"].append({'name': target_buff_item_name, 'cost': buff_cost, 'type': 'buff_item_sfl'}) 
                        action_taken_this_day_in_purchase_loop = True
                        break
                        ...
                        purchased_buff_items_globally[target_buff_item_name] = True
                        daily_log_entry["purchases_today"].append({'name': target_buff_item_name, 'cost': buff_cost, 'type': 'buff_item_token'})
                        log.info(f"[CustomCal] Dia {day_in_season_counter}: Comprado buff de token selecionado '{target_buff_item_name}'. Custo: {buff_cost}")
                        action_taken_this_day_in_purchase_loop = True
                        break
        ...
        # Reseta antes de recalcular com os buffs do dia
        daily_log_entry["gains_item_bonuses_detailed"] = {} 
        for buff_item_name_init_recalc in all_buffs_to_track_totals: # Use a lista completa de buffs rastreáveis
            daily_log_entry["item_specific_bonus_totals"][buff_item_name_init_recalc] = 0

        current_day_vip_deliveries_bonus = 0 
        current_day_vip_chores_bonus = 0

        # Ganhos base (já calculados antes do loop de compra, mas precisam ser referenciados)
        # Se as atividades ainda não começaram, os ganhos base são zero.
        gains_chest_today = SIM_DAILY_CHEST_REWARD_TOKENS if current_date_iterator >= activities_start_date_obj else 0
        base_delivery_gains_today = SIM_DAILY_NPC_DELIVERIES_BASE_TOKENS if current_date_iterator >= activities_start_date_obj else 0
        base_chores_gains_today = SIM_WEEKLY_CHORES_BASE_TOKENS if current_date_iterator >= activities_start_date_obj and day_of_week_num == 6 else 0
        base_megaboard_gains_today = SIM_WEEKLY_MEGABOARD_BOUNTIES_BASE_TOKENS if current_date_iterator >= activities_start_date_obj and day_of_week_num == 0 else 0
        base_animal_gains_today = SIM_WEEKLY_ANIMAL_BOUNTIES_BASE_TOKENS if current_date_iterator >= activities_start_date_obj and day_of_week_num == 0 else 0
        megaboard_completion_bonus_today = SIM_WEEKLY_MEGABOARD_COMPLETION_BONUS_TOKENS if current_date_iterator >= activities_start_date_obj and day_of_week_num == 0 and SIM_IDEAL_PLAYER_ACHIEVES_MEGA_BOUNTY_BONUS else 0

        daily_log_entry["gains_chest"] = gains_chest_today
        delivery_multiplier = 2 if is_double_day else 1
        daily_log_entry["gains_deliveries_base"] = base_delivery_gains_today * delivery_multiplier
        daily_log_entry["gains_chores_base"] = base_chores_gains_today
        daily_log_entry["gains_megaboard_base"] = base_megaboard_gains_today
        daily_log_entry["gains_animals_base"] = base_animal_gains_today
        daily_log_entry["gains_megaboard_completion_bonus"] = megaboard_completion_bonus_today

        if current_date_iterator >= activities_start_date_obj:
            if SIM_IDEAL_PLAYER_HAS_VIP:
                if "vip" in activity_rules_global.get("deliveries",{}).get("applicable_bonuses",[]):
                    current_day_vip_deliveries_bonus = (vip_bonus_val * SIM_DAILY_NPC_DELIVERIES_COUNT) * delivery_multiplier
                if day_of_week_num == 6 and "vip" in activity_rules_global.get("chores",{}).get("applicable_bonuses",[]):
                    current_day_vip_chores_bonus = vip_bonus_val * SIM_WEEKLY_CHORES_COUNT
            daily_log_entry["gains_deliveries_vip_bonus"] = current_day_vip_deliveries_bonus
            daily_log_entry["gains_chores_vip_bonus"] = current_day_vip_chores_bonus

            # Bônus de itens para entregas
            for buff_name in active_buffs_at_day_start:
                item_def = buff_definitions_global.get(buff_name, {})
                item_bonus_value = item_def.get("bonus_value", 0)
                if item_bonus_value > 0 and buff_name in activity_rules_global.get("deliveries",{}).get("applicable_bonuses",[]):
            ...
            # Bônus de itens para chores (usando buffs ativos na semana do chore)
            if day_of_week_num == 6: # Chores são no Domingo (6)
                # Atualiza buffs_for_current_chores_week com os buffs ativos no dia do chore
                buffs_for_current_chores_week = set(active_buffs_at_day_start)
                for buff_name in buffs_for_current_chores_week:
                    item_def = buff_definitions_global.get(buff_name, {})
                    item_bonus_value = item_def.get("bonus_value", 0)
            ...
            # Bônus de itens para bounties (usando buffs ativos na semana do bounty)
            if day_of_week_num == 0: # Bounties são na Segunda (0)
                # Atualiza buffs_for_current_bounties_week com os buffs ativos no dia do bounty
                buffs_for_current_bounties_week = set(active_buffs_at_day_start)
                for buff_name in buffs_for_current_bounties_week:
                    item_def = buff_definitions_global.get(buff_name, {})
                    item_bonus_value = item_def.get("bonus_value", 0)
        ...
        daily_log_entry["total_gains_today"] = gains_today_gross
        ...
        # O current_token_balance já foi ajustado durante as compras de token.
        # Aqui, adicionamos os ganhos brutos ao saldo que resultou das compras.
        # Saldo inicial do dia (que já reflete compras de dias anteriores)
        # + Ganhos brutos do dia
        # - Custo de tokens de compras FEITAS HOJE (já deduzido de current_token_balance no loop de compra)
        # Portanto, o saldo final é o current_token_balance (após compras) + ganhos brutos.
        # No entanto, a forma como current_token_balance é atualizado no loop de compra e como
        # daily_log_entry["balance_start_day"] é usado precisa ser consistente.

        # Correção: current_token_balance é o saldo *após* as compras de tokens do dia.
        # Ganhos brutos são adicionados a este saldo.
        current_token_balance += gains_today_gross # Adiciona ganhos ao saldo que já teve custos de token deduzidos
        daily_log_entry["balance_end_day"] = round(current_token_balance, 2)
        
        # --- FIM DA LÓGICA DE RECÁLCULO DE GANHOS ---

        daily_log_entry["active_buffs_str"] = ", ".join(sorted(list(purchased_buff_items_globally.keys())))
        calendar_data_list.append(daily_log_entry)
        current_date_iterator += datetime.timedelta(days=1)
    
    log.info(f"Geração do calendário customizado finalizada. {len(calendar_data_list)} dias processados.")
    return calendar_data_list
...
if __name__ == '__main__':
    ...
    # generated_calendar = generate_custom_season_calendar(user_selected_buff_names=["Flower Mask", "Heart Air Balloon"])
    generated_calendar = generate_max_potential_season_calendar() # Teste da original
